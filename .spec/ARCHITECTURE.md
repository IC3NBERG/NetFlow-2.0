...
│   ├── customization/      # Sidebar order, colori, tema
...
│   ├── notifications/      # Pagina storico notifiche
...
│   ├── hooks/              # useJobs, useQuotes, useClients, useInvoices, useNotifications, useRealtimeNotifications, useRealtimeSync, useOsNotifications, useFaviconBadge, useNotificationSound, useNotificationCleanup, useGoalNotifications, useNavigationDirection, useDashboardData, useCreateInvoiceWithJobs, useUserSettings, useFiscalSetup, useShares, useSharedData, ecc.
│   ├── stores/             # Zustand stores (fiscalYear, jobsUI, customization, ui, ecc.)
...
**Realtime sync → Notifications:** Il `SyncProvider` crea una notifica di categoria `sync` quando la connessione torna online e la coda di sync viene svuotata.
...
- `NotificationsPage`: Pagina `/notifications` con storico completo, filtri per categoria, toggle mostra archiviate, badges conteggio per categoria, animazioni Framer Motion.
...
### 3.6 Page Transitions (Route Animation) — Eager-loaded, no Suspense
...
- **Eager-loaded (v0.44.5):** Tutte le pagine sono importate eager (no `React.lazy()` / `Suspense`). Il cambio pagina è istantaneo — il componente della nuova pagina è già in memoria.
- **Key change** → React smonta immediatamente la vecchia pagina (nessuna exit animation) e monta la nuova con entrance spring "sollevamento".
- **Spring physics**: `stiffness: 280, damping: 24, mass: 1.1` per effetto di sollevamento premium.
- **Affidabilità**: nessun `AnimatePresence` o freezing di contenuto.
...
1. **`transition-property: background-color, color, border-color`** su `body` (0.4s) con `cubic-bezier(0.16, 1, 0.3, 1)` — Material Design "emphasized deceleration": partenza rapida, decelerazione che si addolcisce.
2. **Class toggle** — il `ThemeProvider` commuta `.dark`/`.light` su `<html>`, i CSS variables cambiano istantaneamente, e le transizioni morphiano i colori su tutti gli elementi. Nessun rimbalzo (`theme-breath` rimosso in v0.44.2).
...
### 3.8 Dashboard Module Animation
...
### 3.9 Componenti Grafici Avanzati
