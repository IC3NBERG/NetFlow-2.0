        ...
        <div className="mx-auto flex max-w-6xl items-center gap-4 px-6 py-5">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2.5">
              <h1 className="text-lg font-bold text-text-primary truncate">{businessName}</h1>
              <span className="inline-flex items-center gap-1 rounded-full bg-success/15 border border-success/35 px-2.5 py-0.5 text-[10px] font-bold text-success uppercase tracking-wider">
            ...
            <p className="mt-1 flex items-center gap-2 text-xs text-text-secondary">
              ...
              {shareInfo?.view_count !== undefined && (
                <>
                  ...
                  <span>Visite: <strong className="text-text-primary">{shareInfo.view_count}</strong></span>
                </>
              )}
            </p>
          </div>
          {shareInfo?.access_level === 'export' && (
            <span className="hidden sm:flex items-center gap-1.5 rounded-full bg-brand/10 px-3.5 py-1.5 text-xs font-semibold text-brand border border-brand/25 shadow-sm shadow-brand/5 uppercase tracking-wider">
              <ExternalLink className="h-3.5 w-3.5" /> Export abilitato
            </span>
          )}
        </div>
      </header>
            ...
            {stats.map((stat) => {
              ...
              return (
                <motion.div
                  key={stat.label}
                  variants={itemVariants}
                  className="group relative overflow-hidden rounded-[1.25rem] bg-surface/40 backdrop-blur-2xl border border-border/50 p-4 transition-all duration-300 hover:border-brand/40 hover:scale-[1.02] hover:shadow-[0_8px_30px_rgba(197,150,58,0.05)]"
                >
                  <div className={`absolute right-0 top-0 h-24 w-24 -translate-y-6 translate-x-6 rounded-full opacity-[0.05] transition-opacity duration-300 group-hover:opacity-[0.08] ${stat.bgColor}`} />
                  <div className="relative">
                    <div className={`mb-3 inline-flex rounded-xl ${stat.bgColor}/10 p-2 ring-1 ring-inset ring-border/40`}>
                      <Icon className={`h-4.5 w-4.5 ${stat.color}`} />
                    </div>
                    <p className="text-2xl font-bold font-mono tabular-nums text-text-primary">{stat.value}</p>
                    <p className="mt-0.5 text-xs font-medium text-text-secondary">{stat.label}</p>
                  </div>
                </motion.div>
              )
            })}
